...
export const docsOnboardingHierarchy = {
  ...
  html: `<p>BuildSync enforces a structured onboarding hierarchy — each role can only add users below them in the chain:</p><ul><li><strong>Organization Admin / Building Owner (Tier 1)</strong> → adds Building Managers, Facility Managers (Full Manager Access).</li><li><strong>Building Manager (Tier 3 — Super User)</strong> → adds Security, Concierge, Staff, Residents, Tenants (Operational / Resident Portal Access).</li><li><strong>Facility Manager (Tier 2)</strong> → adds Vendors, Contractors, Maintenance Staff (Work-Order Only Access).</li></ul><p>Three onboarding flows: (1) Self-Onboarding via magic links / invite codes, (2) Bulk CSV Import for 200+ residents, (3) RBAC role templates for automatic permission assignment. Invite codes are 8-character alphanumeric, valid for 7 days.</p>`
};
